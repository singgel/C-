...
namespace CertificateWebCrawlerSys
{
    public class HGZUtils
    {
        ...
        public HGZUtils(string name, GridControl gc, ListBox lb)
        {
            this.gc = gc;
            this.HtmlName = name;
            this.CrawlerLog = lb;
        ...
        public HGZUtils(string name, GridControl gc, Label lbl)
        {
            this.gc = gc;
            this.HtmlName = name;
            this.lbl = lbl;
        ...
        public void Start(string ids)
        {
            ...
            try
            {
                if (IsStopped)
                {
                    Thread thAutoCrawlerPageList;
                    thAutoCrawlerPageList = new Thread(new ParameterizedThreadStart(AutoCrawlerDataList));  //获取线程
                    thAutoCrawlerPageList.IsBackground = true;
                    thAutoCrawlerPageList.Start(ids);
                }
            }
            catch (System.Exception ex)
            {
        ...
        private void AutoCrawlerDataList(object oPage)
        {
            string msg = string.Empty;
            //获取最大页数
            string MaxPageNumm = Tool.GetHtmlSourceListMaxPageNum(Tool.strTargerUrlListHGZ + "1");
            ...
            try
            {
                string[] dTimes = oPage.ToString().Split(',');
        ...
        /// 按照指定的页数抓取数据：
        /// </summary>
        /// <param name="dTime">页数</param>
        private void AutoCrawlerDataOne(int page)
        {
        ...
        public void InsertDataHGZPool(string strTargerUrl, DataTable data, int page)
        {
            ...
            {
                msg = string.Format("{0} 正在抓取第{1}页，插入合格证详细信息数据时出错：i={2},{3}", DateTime.Now.ToString("G"), page.ToString(), i.ToString(), ex.Message);
                LogWrite(msg);
            }
        }
