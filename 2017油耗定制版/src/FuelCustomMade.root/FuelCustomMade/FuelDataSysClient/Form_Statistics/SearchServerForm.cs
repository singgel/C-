...
namespace FuelDataSysClient.Form_Statistics
{
    ...
    public partial class SearchServerForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void SearchServerForm_Load(object sender, EventArgs e)
        {
        ...
        private void barPrint_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            foreach (VehicleBasicInfo vbi in dvList)
            {
                if (vbi.Check == true)
                {
                    ...
                    printModel.Zczbzl = vbi.Zczbzl;
                    string strRllx = vbi.Rllx;

                    printModel.Qdxs = vbi.Qdxs;
                    printModel.Zdsjzzl = vbi.Zdsjzzl;
        ...
        private void barExcel_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
                ...
                if (Utils.userId.Equals("FADCFFTZGU001"))
                {
                    ...
                    string sqlVinsCTNY = string.Format(@"select VIN from ({0}) where RLLX in ('汽油','柴油','两用燃料','双燃料') ", sqlVins);
        ...
        //给窗体-控件-赋值-是否可以编辑
        public void setControlValue(FuelDataSysClient.Form_DBManager.JbxxViewForm jvf, string cName, String val, bool enable)
        {
            if (cName == null || "" == cName)
            {
                return;
        ...
        private void GetDataFromService(int pageNum)
        {
            ...
            try
            {
                ...
                FuelDataService.VehicleBasicInfo[] queryInfoArr = Utils.service.QueryUploadedFuelData_New(Utils.userId, Utils.password, pageNum, pageSize, tbVin.Text, tbClxh.Text, tbClzl.Text, cbRllx.Text, dtStartTime.Text, dtEndTime.Text, this.GetTimeType());
                if ((queryInfoArr != null && dataCount > 0) || (queryInfoArr == null && dataCount == 0))
                {
                    if (queryInfoArr != null)
                    {
                        ...
                        this.dgvCljbxx.DataSource = vbis;
                        this.gvCljbxx.BestFitColumns();
                    }
                    else
                    {
                        this.dgvCljbxx.DataSource = null;
                    }
                    //页码显示信息计算
        ...
        private void LoadRemoteData()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    var fuelData = Utils.service.QueryUploadedFuelData_New(Utils.userId, Utils.password, page, 100, tbVin.Text, this.tbClxh.Text, tbClzl.Text, cbRllx.Text, this.dtStartTime.Text, this.dtEndTime.Text, timeType);
                    exception = false;
                    if (fuelData != null)
                    {
