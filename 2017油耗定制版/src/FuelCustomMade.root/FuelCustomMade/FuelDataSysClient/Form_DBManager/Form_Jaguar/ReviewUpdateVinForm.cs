...
namespace FuelDataSysClient.Form_DBManager.Form_Jaguar
{
    public partial class ReviewUpdateVinForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private string cocIds;
        private Dictionary<string, string> dictStatus;
        private Dictionary<string, string> dictVinCocId;
        ...
        public ReviewUpdateVinForm(string cocIds)
        {
            InitializeComponent();
            this.cocIds = cocIds;
        ...
        // 查询
        private void SearchLocal(string cocIds)
        {
            // 获取本地车辆基本信息
            string sql = string.Format(@"SELECT * FROM FC_CLJBXX WHERE COC_ID IN ({0})", cocIds);
            string sw = "";
        ...
        protected void GetNewData(DataTable dtUpateVin)
        {
            ...
            dictVinCocId = new Dictionary<string, string>();
            ...
            foreach (DataRow dr in dtUpateVin.Rows)
            {
                ...
                // vin对应的参数编号
                dictVinCocId.Add(vin, dr["COC_ID"] == null ? "" : dr["COC_ID"].ToString());

            }
        }

        // 获取主表数据
        protected DataTable GetCocData(string fuelType)
        {
            string sqlCoc = string.Empty;
            if (fuelType == Utils.CTNY)
            {
                sqlCoc = string.Format(@"SELECT * FROM COC_INFO");
            }
            if (fuelType == Utils.FCDS)
            {
                sqlCoc = string.Format(@"SELECT * FROM COC_FCDS");
            }

            DataTable dt = AccessHelper.ExecuteDataSet(AccessHelper.conn, sqlCoc, null).Tables[0]; ;

            return dt;
        }

        protected string UpdateFuelData(string vin, string status, DataRow drCoc, DataTable dtPam, OleDbConnection con)
        {
            string message = string.Empty;
            ...
            try
            {
                ...
                string sqlUpdateBasic = @"UPDATE FC_CLJBXX SET 
                                    QCSCQY=@QCSCQY,JKQCZJXS=@JKQCZJXS,CLXH=@CLXH,CLZL=@CLZL,
                                    RLLX=@RLLX,ZCZBZL=@ZCZBZL,ZGCS=@ZGCS,LTGG=@LTGG,ZJ=@ZJ,
                                    TYMC=@TYMC,YYC=@YYC,ZWPS=@ZWPS,ZDSJZZL=@ZDSJZZL,EDZK=@EDZK,LJ=@LJ,
                                    QDXS=@QDXS,JYJGMC=@JYJGMC,JYBGBH=@JYBGBH,QTXX=@QTXX,STATUS=@STATUS,UPDATETIME=@UPDATETIME
                                   WHERE VIN=@VIN";
                ...
                OleDbParameter[] parameters = {
					        new OleDbParameter("@QCSCQY", drCoc["QCSCQY"].ToString()),
					        new OleDbParameter("@JKQCZJXS", drCoc["JKQCZJXS"].ToString()),
					        new OleDbParameter("@CLXH", drCoc["CLXH"].ToString()),
					        new OleDbParameter("@CLZL", drCoc["CLZL"].ToString()),

					        new OleDbParameter("@RLLX", drCoc["RLLX"].ToString()),
					        new OleDbParameter("@ZCZBZL", drCoc["ZCZBZL"].ToString()),
					        new OleDbParameter("@ZGCS", drCoc["ZGCS"].ToString()),
					        new OleDbParameter("@LTGG", drCoc["LTGG"].ToString()),
					        new OleDbParameter("@ZJ", drCoc["ZJ"].ToString()),

					        new OleDbParameter("@TYMC", drCoc["TYMC"].ToString()),
					        new OleDbParameter("@YYC", drCoc["YYC"].ToString()),
					        new OleDbParameter("@ZWPS", drCoc["ZWPS"].ToString()),
					        new OleDbParameter("@ZDSJZZL", drCoc["ZDSJZZL"].ToString()),
					        new OleDbParameter("@EDZK", drCoc["EDZK"].ToString()),
					        new OleDbParameter("@LJ", drCoc["LJ"].ToString()),

					        new OleDbParameter("@QDXS", drCoc["QDXS"].ToString()),
					        new OleDbParameter("@JYJGMC", drCoc["JYJGMC"].ToString()),
					        new OleDbParameter("@JYBGBH", drCoc["JYBGBH"].ToString()),
					        new OleDbParameter("@QTXX", drCoc["CT_QTXX"].ToString()),
                ...
                foreach (DataRow drParam in dtPam.Rows)
                {
                    ...
                    OleDbParameter[] paramList = { 
                                     new OleDbParameter("@PARAM_VALUE",drCoc[paramCode]),
                                     new OleDbParameter("@VIN",vin),
