    ...
    public partial class ReviewUpdateVinForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void SearchLocal(string mainIds)
        {
            ...
            try
            {
                ...
                {
                    sw += String.Format(" AND (CLZZRQ>=#{0}#)", Convert.ToDateTime(this.dtStartTime.Text));
                }
                if (!string.IsNullOrEmpty(this.dtEndTime.Text))
                {
                    sw += String.Format(" AND (CLZZRQ<#{0}#)", Convert.ToDateTime(this.dtEndTime.Text).Add(new TimeSpan(24, 0, 0)));
                }
                var dt = AccessHelper.ExecuteDataSet(AccessHelper.conn, sql + sw + sqlOrder, null).Tables[0];
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            DataTable dtCtnyPam = mitsUtil.GetRllxData("传统能源");
