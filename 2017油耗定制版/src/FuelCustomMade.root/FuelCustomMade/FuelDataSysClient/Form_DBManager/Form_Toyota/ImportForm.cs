...
namespace FuelDataSysClient.Form_DBManager.Form_Toyota
{
    public partial class ImportForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        {
            if (this.btnFcdsSearch.SelectedTabPage.Text.Equals("传统能源"))
            {
            ...
            if (this.btnFcdsSearch.SelectedTabPage.Text.Equals("非插电式混合动力"))
            {
        ...
        private void btnReviewMainData_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ReviewMainData cocForm = new ReviewMainData();
                cocForm.ShowDialog();
            }
            catch (Exception ex)
            {
        ...
        protected DataSet GetReadyDataFromLocalCoc()
        {
            ...
            try
            {
                string sql = @"SELECT VI.VIN,VI.CLZZRQ,CI.* FROM VIN_INFO VI "
                                + " LEFT JOIN MAIN_CTNY CI ON CI.MAIN_ID=VI.MAIN_ID "
                                + " WHERE CI.STATUS='1'";
        ...
        protected bool CheckIfVinDataReady()
        {
            ...
            string sqlQuery = @"SELECT VI.VIN FROM VIN_INFO VI";
            try
            {
        ...
        protected bool CheckIfCocDataReady()
        {
            ...
            string sqlCocQuery = @"SELECT CI.MAIN_ID FROM MAIN_CTNY CI WHERE CI.MAIN_ID IN 
                                (SELECT DISTINCT(VI.MAIN_ID) FROM VIN_INFO VI)";
            string sqlQuery = @"SELECT T1.MAIN_ID FROM (SELECT DISTINCT(MAIN_ID) FROM VIN_INFO) T1 
                                WHERE T1.MAIN_ID NOT IN ((SELECT CI.MAIN_ID FROM MAIN_CTNY CI))";
            try
            {
