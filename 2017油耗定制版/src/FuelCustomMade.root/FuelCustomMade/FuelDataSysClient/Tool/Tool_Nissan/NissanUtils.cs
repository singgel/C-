...
namespace FuelDataSysClient.Tool.Tool_Nissan
{
        ...
        try
        {
            foreach (DataRow edr in dr)
            {
                ...
                {
                    case "CDD_DLXDCBNL":
                        message += VerifyDLXDCBNL("动力蓄电池组比能量", Convert.ToString(r[code]));
                        break;
                    case "CDD_DLXDCZEDNL":
                        message += VerifyFloat("动力蓄电池组总能量", Convert.ToString(r[code]));
                        break;
                    case "CDD_DDXDCZZLYZCZBZLDBZ":
                        message += VerifyDLXDCBNL("动力蓄电池总质量与整车整备质量的比值", Convert.ToString(r[code]));
                        break;
                    case "CDD_DLXDCZBCDY":
                    ...
                    case "CDD_DDQC30FZZGCS":
                        ...
                        break;
                    case "CDD_ZHGKXSLC":
                        message += VerifyDLXDCBNL("综合工况续驶里程", Convert.ToString(r[code]));
                        break;
                    case "CDD_QDDJFZNJ":
            ...
            foreach (DataRow edr in dr)
            {
                ...
                switch (code)
                {
                    ...
                    case "FCDS_HHDL_CDDMSXZHGKXSLC":
                        message += VerifyDLXDCBNL("纯电动模式下综合工况续驶里程", Convert.ToString(r[code]));
                        break;
            ...
            foreach (DataRow edr in dr)
            {
                ...
                switch (code)
                {
                    ...
                    case "RLDC_DDHHJSTJXXDCZBNL":
                        message += VerifyDLXDCBNL("电电混合技术条件下动力蓄电池组比能量", Convert.ToString(r[code]));
                        break;
                    case "RLDC_ZHGKHQL":
                        message += VerifyFloat("综合工况燃料消耗量", Convert.ToString(r[code]));
                        break;
        ...
        // 验证动力蓄电池组比能量和纯电动模式下综合工况续驶里程,综合工况续驶里程
