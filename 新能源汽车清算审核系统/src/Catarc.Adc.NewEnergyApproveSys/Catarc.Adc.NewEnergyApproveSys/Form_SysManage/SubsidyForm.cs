...
{
    public partial class SubsidyForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        public SubsidyForm()
        {
            InitializeComponent();
        ...
        private void barBtnImportNew_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                try 
                {
                    ...
                    Utils ut = new Utils();
                    //导入数据
                    returnMsg += ut.ImportSubsidyData(openFileDialog.FileName);
        ...
        {
            string sqlCount = String.Format("select count(*) from DB_SUBSIDY where 1=1 {0}", this.queryParam());
        ...
        private DataTable queryByPage(int pageNum)
        {
            ...
            string sqlVins = string.Format(@"select * from DB_SUBSIDY where 1=1 {0}", sqlWhere);
        ...
        {
            string sqlAll = String.Format("select * from DB_SUBSIDY where 1=1 {0}", this.queryParam());
        ...
        private string queryParam()
        {
            ...
            {
                sqlStr.Append(string.Format(@" AND to_date(to_char(TJMLFBSJ,'yyyy/MM/dd'),'yyyy-mm-dd hh24:mi:ss') >= to_date('{0}','yyyy-mm-dd hh24:mi:ss') AND to_date(to_char(TJMLFBSJ,'yyyy/MM/dd'),'yyyy-mm-dd hh24:mi:ss')< to_date('{1}','yyyy-mm-dd hh24:mi:ss') ", Year.Text + "-01-01", (int.Parse(Year.Text) + 1).ToString() + "-01-01"));
            }
        ...
        //删除
        private void bar_Delete_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            using (OracleConnection conn = new OracleConnection(OracleHelper.conn))
            {
                ...
                using (OracleTransaction trans = conn.BeginTransaction())
                {
                    foreach (DataRow dr in dtSelected.Rows)
                    {
                        try
                        {
                            OracleHelper.ExecuteNonQuery(trans, string.Format("DELETE FROM DB_SUBSIDY WHERE　CLSCQY = '{0}' AND CLXH = '{1}'AND BTBZ = '{2}' AND TJMLFBSJ = to_date('{3}','yyyy-mm-dd hh24:mi:ss')  ", dr["CLSCQY"], dr["CLXH"], dr["BTBZ"], dr["TJMLFBSJ"]), null);
                        }
                        catch (Exception ex)
                        {
                            trans.Rollback();
            ...
            //记录操作日志
            LogUtils.ReviewLogManager.ReviewLog(Properties.Settings.Default.LocalUserName, String.Format("{0}-{1}", this.Text, this.bar_Delete.Caption));
