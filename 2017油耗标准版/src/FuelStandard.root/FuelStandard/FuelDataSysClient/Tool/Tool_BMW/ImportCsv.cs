...
namespace FuelDataSysClient.Tool.Tool_BMW
{
    ...
    {
        private string fileMark = "*.csv";
        Dictionary<string, string> dictionary = new Dictionary<string, string>();
        string strCon = AccessHelper.conn;
        DataTable checkData = new DataTable();
        string path = Application.StartupPath + Settings.Default["ExcelHeaderTemplate_BMW"];
        ...
        public Dictionary<string, Dictionary<string, string>> ReadCsv(string filePath)
        {
            ...
            try
            {
                ...
                if (fileNameList.Count > 0)
                {
                    ...
                    {
                        bool result = false;
                        DataTable dt = ReadCsvFileToTable(true, ',', str);
                    ...
                    using (OleDbTransaction tra = con.BeginTransaction())
                    {
                        try
                        {
                            ...
                            foreach (DataRow r in dt.Rows)
                            {
                                ...
                                parametersFC_CLJBXX[27].Value = "";//r["QTXX"]
        ...
        public DataSet ReadSharchExcel(string path, string sheet, string status)
        {
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(r["VIN"].ToString()))
                    {
                        strAdd.Append("'");
