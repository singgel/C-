...
namespace FuelDataSysServer.Tool
{
    class AutoSocket
    {
        ...
        //生产线1 
         string listenerPort1 = FuelDataSysServer.Properties.Settings.Default.SocketPort1;
         Socket listener1;
         private byte[] m_byBuff1 = new byte[45];


         //生产线2-1
         string listenerPort2_1 = FuelDataSysServer.Properties.Settings.Default.SocketPort2_1;
         Socket listener2_1;
         private byte[] m_byBuffRecieve2_1 = new byte[500];
         private byte[] m_byBuffSend2_1 = new byte[500];

         //生产线2-2
         string listenerPort2_2 = FuelDataSysServer.Properties.Settings.Default.SocketPort2_2;
         Socket listener2_2;
         ...
         //生产线3_1 
         string listenerPort3_1 = FuelDataSysServer.Properties.Settings.Default.SocketPort3_1;
         Socket listener3_1;
         ...
         //生产线3_2 
         string listenerPort3_2 = FuelDataSysServer.Properties.Settings.Default.SocketPort3_2;
         Socket listener3_2;
        ...
        public void ListenClientConnect2_1()
        {
            ...
            while (true)
            {
                ...
                try
                {
                   ...
                   clientSocket = listener2_1.Accept();
                    var remoteIP = ((IPEndPoint)clientSocket.RemoteEndPoint);
                    localIP = ((IPEndPoint)clientSocket.LocalEndPoint);
                    strAddress = remoteIP.Address.ToString();
                    Port = remoteIP.Port;
        ...
        public void ListenClientConnect2_2()
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    clientSocket = listener2_2.Accept();
                    var remoteIP = ((IPEndPoint)clientSocket.RemoteEndPoint);
                    localIP = ((IPEndPoint)clientSocket.LocalEndPoint);
                    strAddress = remoteIP.Address.ToString();
                    Port = remoteIP.Port;
            ...
            while (true)
            {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        //等待连接
                        clientSocket = listener3_1.Accept();
                        var remoteIP = ((IPEndPoint)clientSocket.RemoteEndPoint);
                        localIP = ((IPEndPoint)clientSocket.LocalEndPoint);
                        strAddress = remoteIP.Address.ToString();
                        Port = remoteIP.Port;
            ...
            while (true)
            {
                ...
                do
                {
                    ...
                    try
                    {
                        ...
                        //等待连接
                        clientSocket = listener3_2.Accept();
                        var remoteIP = ((IPEndPoint)clientSocket.RemoteEndPoint);
                        localIP = ((IPEndPoint)clientSocket.LocalEndPoint);
                        strAddress = remoteIP.Address.ToString();
                        Port = remoteIP.Port;
                ...
                using (OracleConnection con = new OracleConnection(OracleHelper.conn))
                {
                    ...
                    try
                    {
                        ...
                        string sqlInsert = string.Format("INSERT INTO VIN_INFO(ID,VIN,SC_OCN,CLZZRQ,IPV4,PORT,MERGER_STATUS,"
                                                        ...
                                                        DateTime.Now.ToString("yyyy-MM-dd HH：mm：ss"), receiveStr, baseID);
