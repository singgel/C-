...
namespace FuelDataSysServer.Tool
{
    class UploadData
    {
        ...
        {
            DataSet ds = OracleHelper.ExecuteDataSet(OracleHelper.conn, string.Format(@"SELECT VIN FROM FC_CLJBXX WHERE STATUS='1' AND UPLOADDEADLINE>to_date('{0}','yyyy-mm-dd hh24:mi:ss') ", DateTime.Today.ToString("yyyy-MM-dd HH：mm：ss")), null);
        ...
        {
            DataSet ds = OracleHelper.ExecuteDataSet(OracleHelper.conn, string.Format(@"SELECT VIN FROM FC_CLJBXX WHERE STATUS='1' AND UPLOADDEADLINE>to_date('{0}','yyyy-mm-dd hh24:mi:ss') AND CREATETIME<=to_date('{1}','yyyy-mm-dd hh24:mi:ss')", DateTime.Today.ToString("yyyy-MM-dd HH：mm：ss"), Utils.QueryUploadLimitLine(DateTime.Today)), null);
            if (ds != null && ds.Tables.Count > 0)
            {
                return ds.Tables[0];
            }
            ...
            try
            {
                ...
                if (vinList.Count > 0)
                {
                    for (int i = 0; i < vinList.Count; )
                    {
                        ...
                        result = Utils.service.UploadInsertFuelDataList(Utils.userId, Utils.userPWD, Utils.FuelInfoC2S(vbiList).ToArray(), "CATARC_CUSTOM_2012");
            ...
            try
            {
                ...
                if (vinList.Count > 0)
                {
                    for (int i = 0; i < vinList.Count; )
                    {
                        ...
                        List<VehicleBasicInfo> vbiList = Utils.GetApplyParam(tempList);
                        // 上传
                        result = Utils.service.UploadOverTime(Utils.userId, Utils.userPWD, Utils.FuelInfoC2S(vbiList).ToArray(), "CATARC_CUSTOM_2012");
            ...
            try
            {
                ...
                foreach (string vins in arrVin)
                {
                    ...
                    if (!string.IsNullOrEmpty(vins))
                    {
                        ...
                        DataSet tempDt = Utils.service.QueryVidByVins(Utils.userId, Utils.userPWD, vin);
                        if (tempDt != null)
                        {
                ...
                using (OracleTransaction tra = con.BeginTransaction())
                {
                    try
                    {
                        foreach (DataRow dr in dt.Rows)
                        {
                            ...
                            {
                                this.userLoginLog.Items.Add(String.Format("{0:G}== 自动上报 VIN:{1} 已经上报，反馈码为:{2} ==", DateTime.Now.ToString("yyyy-MM-dd HH：mm：ss"), dr[0], dr[1]));
