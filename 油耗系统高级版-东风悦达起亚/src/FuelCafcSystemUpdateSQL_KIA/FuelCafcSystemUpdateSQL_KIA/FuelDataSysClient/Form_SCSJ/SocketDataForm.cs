...
{
    public partial class SocketDataForm : DevExpress.XtraBars.Ribbon.RibbonForm
    {

        public SocketDataForm()
        {
            InitializeComponent();
            //初始时间和日期
        ...
        private void SocketDataForm_Load(object sender, EventArgs e)
        {
            try
            {
                if (FuelDataSysClient.Tool.OracleHelper.Exists(FuelDataSysClient.Tool.OracleHelper.conn, "select count(*) from SYS_AUTOMATIC where STATIC=1 and AUTOTYPE!='IsAutoUpload'"))
                {
